...
window.addEventListener('scroll', function () {
    ...
    const maxPosition = 100; // Replace with the maximum 'value' you expect
    const minPosition = 0;   // Replace with the minimum 'value' you expect
    

    // Calculate opacity
    let normalizedValue = (value - minPosition) / (maxPosition - minPosition); 

    normalizedValue = Math.max(0, Math.min(1, normalizedValue)); // Clamp between 0 and 1

cloud.style.opacity = normalizedValue;
    bird1.style.top = value * 0.1 + 'px';
    bird1.style.left = value * 13 + 'px';
   
normalizedValue = Math.max(0, Math.min(1, normalizedValue)); // Clamp between 0 and 1

bird1.style.opacity = normalizedValue;

    bird2.style.top = value * 2.5 + 'px';
    // bird2.style.left = value * -2 + 'px';
 // Replace with the minimum 'value' you expect
    
    // Calculate opacity
    let normalizedValue2 = (value - minPosition) / (maxPosition - minPosition); 
    normalizedValue = Math.max(0, Math.min(1, normalizedValue)); // Clamp between 0 and 1
    
    bird2.style.opacity = normalizedValue;
...
function openlink(x) {
    if (x == 1) {
        window.open("https://www.instagram.com/.vini._02/", "_blank");
    }
    if (x == 2) {
